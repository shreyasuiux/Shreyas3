...
function Nav({ onWhoWeAreItemClick, onLogoClick, onGetStartedClick, onSearchClick }: { onWhoWeAreItemClick?: (item: string) => void; onLogoClick?: () => void; onGetStartedClick?: () => void; onSearchClick?: () => void }) {
  ...
  return (
    <>
      <div className="hidden md:flex fixed backdrop-blur-[25px] backdrop-filter bg-[rgba(0,0,0,0.5)] content-stretch flex-col h-[92px] items-start justify-center left-0 right-0 overflow-visible px-[80px] py-[12px] shadow-[0px_2px_30px_0px_rgba(0,0,0,0.05),0px_8px_72px_-5px_rgba(0,0,0,0.1)] top-0 z-[998]" data-name="Nav">
...
export default function Desktop2({ onSearchClick }: { onSearchClick?: () => void }) {
  ...
  // Case Studies now uses React Router - this is a no-op for backward compatibility
  const setShowCaseStudies = () => {};
  ...
  const handleGrowWithUsClick = () => {
    // Navigate to Case Studies page using React Router
    window.scrollTo({ top: 0, behavior: 'auto' });
    navigate('/case-studies');
  };
